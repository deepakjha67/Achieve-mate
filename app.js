// DATA MODEL - Updated with Pause Logic Fields
window.appData = { 
    playlists: [], dailyGoals: [], streak: 0, lastActive: null, history: {}, 
...
function checkReminders() {
    ...
    // Check only at minute 00 to avoid multiple alerts
    if(min === 0) {
            if(hour === 8 || hour === 20) {
                if (Notification.permission === "granted") {
                new Notification("Achieve-mate", { body: "Time to update your goals and streak!", icon: "" });
            }
            }
    }
}
...
function init() {
    // SAFE DATA LOAD STRATEGY
    const local = localStorage.getItem('achieveMate_v12');
    if(local) {
        try {
            const saved = JSON.parse(local);
            // Merge saved data into default structure to ensure new fields (like focusEndTime) exist
    ...
    // Check Active Focus Session (Resume if valid)
...
window.toggleAuthMode = function(mode) {
    ...
    if(mode === 'signup') {
        ...
        // Fix layout shift: Use visibility instead of display
        ...
        // Fix layout shift: Restore visibility
...
window.toggleGoal = function(id) {
    ...
    if(g) {
        ...
        if(g.done) { 
            ...
            addXP(-xpAmount);
            unlogActivity('Goal', g.text); // FIX DUPLICATE LOGS
        }
        window.saveData();
        window.refreshUI(); 
    }
...
window.toggleVideo = function(plId, vId, event) {
    ...
    if(v.done) { 
        ...
        addXP(-xpAmount);
        unlogActivity('Study', `${pl.name}: ${v.title}`); // FIX DUPLICATE LOGS
    }
...
window.openEditPlaylist = function(id) {
    ...
    // Clear the generator count when opening edit so it doesn't overwrite immediately
...
function renderPlaylists() {
    ...
    let filteredList = window.appData.playlists; // Copy ref
    ...
    // ADD CARD ONLY IF NO PLAYLISTS EXIST (Empty State for New Users)
...
function logActivity(type, title, xp = 0) {
    ...
    if(exists) return; // Don't add if already there
...
function unlogActivity(type, title) {
    ...
    if(window.appData.history[d]) {
        ...
        if(window.appData.history[d].length === 0) delete window.appData.history[d]; // Cleanup empty dates
    }
    renderCalendar();
}

// --- UPDATED FOCUS LOGIC WITH PAUSE/RESUME ---
...
window.toggleFocusPause = function() {
    if (window.appData.isPaused) {
        // RESUME
        window.appData.isPaused = false;
        // Calculate new End Time based on remaining duration
        ...
        // document.getElementById('overlayTimer').textContent remains same
    }
}
...
function resumeFullScreenFocus() {
    ...
    // Check if user loaded page while paused
    if (window.appData.isPaused) {
        document.getElementById('focusPauseBtn').textContent = "Start";
        document.getElementById('focusPauseBtn').classList.remove('btn-warning');
        document.getElementById('focusPauseBtn').classList.add('btn-success');
